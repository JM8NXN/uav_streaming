...
def _flatten_transformer(T, N, x):
    return x.reshape(-1, *x.shape[3:])

def _cast(x):
    return x.transpose(1, 2, 0, 3).reshape(-1, *x.shape[3:])

def _cast_transformer(x):
...
class SharedReplayBuffer(object):
    ...
    def recurrent_generator_transformer(self, advantages, num_mini_batch, data_chunk_length):
        """
        ...
        if len(self.share_obs.shape) > 4:
            ...
            share_obs = _cast_transformer(self.share_obs[:-1])
            obs = _cast_transformer(self.obs[:-1])

        actions = _cast_transformer(self.actions)
        action_log_probs = _cast_transformer(self.action_log_probs)
        advantages = _cast_transformer(advantages)
        value_preds = _cast_transformer(self.value_preds[:-1])
        returns = _cast_transformer(self.returns[:-1])
        masks = _cast_transformer(self.masks[:-1])
        active_masks = _cast_transformer(self.active_masks[:-1])
        # rnn_states = _cast(self.rnn_states[:-1])
        # rnn_states_critic = _cast(self.rnn_states_critic[:-1])
        ...
        if self.available_actions is not None:
            available_actions = _cast_transformer(self.available_actions[:-1])
        ...
        for indices in sampler:
            ...
            # States is just a (N, -1) from_numpy
            rnn_states_batch = np.stack(rnn_states_batch).reshape(N*num_agents, *self.rnn_states.shape[3:])
            rnn_states_critic_batch = np.stack(rnn_states_critic_batch).reshape(N*num_agents, *self.rnn_states_critic.shape[3:])

            # Flatten the (L, N, ...) from_numpys to (L * N, ...)
            share_obs_batch = _flatten_transformer(L, N, share_obs_batch)
            obs_batch = _flatten_transformer(L, N, obs_batch)
            actions_batch = _flatten_transformer(L, N, actions_batch)
            if self.available_actions is not None:
                available_actions_batch = _flatten_transformer(L, N, available_actions_batch)
            else:
                available_actions_batch = None
            value_preds_batch = _flatten_transformer(L, N, value_preds_batch)
            return_batch = _flatten_transformer(L, N, return_batch)
            masks_batch = _flatten_transformer(L, N, masks_batch)
            active_masks_batch = _flatten_transformer(L, N, active_masks_batch)
            old_action_log_probs_batch = _flatten_transformer(L, N, old_action_log_probs_batch)
            adv_targ = _flatten_transformer(L, N, adv_targ)
