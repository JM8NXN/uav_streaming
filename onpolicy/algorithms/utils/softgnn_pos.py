...
class Perception_Graph(torch.nn.Module):
    def __init__(self, num_agents):
        ...
        self.merge = MLP([feature_dim, feature_dim, feature_dim])
    ...
    def forward(self, observations): 
        ...
        uav_uav_dis = self.dis_init(observations['rel_dis'].reshape(rel_shape[0], -1, rel_shape[-1]).transpose(1,2))\
        .reshape(rel_shape[0], -1, rel_shape[1], rel_shape[2])

        edge = self.gnn(pilot_pos, agent_pos, uav_uav_dis)
        # edge = self.merge(edge).reshape(rel_shape[0], -1)
        edge = self.merge(edge)[:, :, 0]  # shape = [batch, 32]
        return edge
